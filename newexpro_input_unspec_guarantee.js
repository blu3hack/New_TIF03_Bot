  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(filePath)
      ...
      .on('data', (data) => {
        ...
        const real = data[Object.keys(data)[3]] || '';

        let newWitel = '';

        if (jenis === 'tif') {
          newWitel = 'TERRITORY ' + witel.padStart(2, '0');
        } else {
...
async function run() {
  try {
    ...
    const files = [
      ['unspec_warranty_tif', 'unspec_warranty', 'tif'],
      ['unspec_warranty_reg', 'unspec_warranty', 'reg'],
      ['unspec_warranty_reg4', 'unspec_warranty', 'reg'],
      ['unspec_warranty_reg5', 'unspec_warranty', 'reg'],
    ];

    for (const [file, table, jenis] of files) {
      await inputDataToDatabase(file, table, jenis);
    }
    await insert_data_ccm('unspec_warranty');
