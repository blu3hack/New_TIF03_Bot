...
async function importCSV(table, fileName) {
  ...
  const selectedCols = ['Workzone', 'Incident', 'cnop3_compliance', 'site_down_severity'];
  const rows = [];
...
async function main() {
  try {
    await deleteData('helper_mttr_mso');
    await importCSV('helper_mttr_mso', 'all-cnop3-ticket');
  } catch (err) {
