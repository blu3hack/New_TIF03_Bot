...
async function importCSV(table, fileName) {
  ...
  const selectedCols = ['INCIDENT', 'WORKZONE', 'WITEL', 'COMPLY_4_E2E', 'COMPLY_24_E2E'];
  const rows = [];
...
async function main() {
  try {
    await deleteData('helper_ttr_indibiz');
    await importCSV('helper_ttr_indibiz', "REPORT_TIKET_COMPLIANCE24_'_'TERRITORY 3'_''_'INDIBIZ_-");
  } catch (err) {
